...
function(input, snames = "", noflevels = NULL, pos = FALSE, ...) {
        ...
        sol <- lapply(unique(lapply(sol, sort)), function(x) {
            ...
            if (pos) {
                ...
                x <- sapply(x, function(y) {
                    ...
                    # print(list(y[2], snames = snames, noflevels = noflevels, scollapse = identical(collapse, "*")))
    ...
    `factorizeit` <- function(x, pos = FALSE, noflevels = NULL, snames = "", mv = FALSE, curly = FALSE) {
        ...
        # return(list(sol = facts, pos = pos, noflevels = noflevels, snames = snames, mv = mv, collapse = collapse))
    ...
    if (is.character(input)) {
        ...
        # return(list(input = input, pos = pos, snames = snames, noflevels = noflevels, mv = mv, curly = curly))
